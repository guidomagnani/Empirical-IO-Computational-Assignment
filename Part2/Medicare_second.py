...
# The  vector  below relates each observation to the market it is in / both for data and demographics
    ...
    for i in range(len(id_demo)):
        ...
        # (was giving value error) # index_demogr=(demogr_year_region_id.loc[demogr_year_region_id[1]==(index_year)[0]].index) & (demogr_year_region_id.loc[demogr_year_region_id[2] == index_region[0] ].index)
        index_demogr= demogr_year_region_id.loc[(demogr_year_region_id[1]==(index_year)[0]) & (demogr_year_region_id[2] == index_region[0] )].index #G's Fix
...
#histogram of the coefficients
       ...
       #######
        ...
        self.theta1_results.to_csv(dir + "theta1_two.csv")
        self.theta2_results.to_csv(dir + "theta2_two.csv")
        self.fval_results.to_csv(dir + "fval_two.csv", index=False)
